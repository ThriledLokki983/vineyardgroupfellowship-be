...
class SecurityValidator:
    ...
    def check_environment_files(self) -> None:
        ...
        try:
            ...
            if env_files:
                self.issues.append(f"❌ .env files are tracked by git: {', '.join(env_files)}")
            else:
        ...
        if env_file.exists():
            ...
            if 'your-secret-key-here' in content:
                self.issues.append("❌ Django SECRET_KEY is still default value")
                
            if 'your_' in content.lower():
                self.warnings.append("⚠️ .env contains template values (your_*)")
    ...
    def _check_settings_file(self, file_path: Path) -> None:
        ...
        if missing_security and 'production' in file_path.name.lower():
            self.warnings.append(f"⚠️ Missing security settings in {file_path.name}: {', '.join(missing_security)}")
    ...
    def check_dependencies(self) -> None:
        ...
        try:
            ...
            if result.returncode != 0:
                if result.stdout:
                    ...
                    try:
                        ...
                        if vulnerabilities:
                            self.issues.append(f"❌ {len(vulnerabilities)} vulnerable dependencies found")
                            for vuln in vulnerabilities[:3]:  # Show first 3
                                self.issues.append(f"   - {vuln.get('package', 'Unknown')} {vuln.get('installed_version', '')}")
                    except json.JSONDecodeError:
                        self.warnings.append("⚠️ Could not parse safety check results")
                else:
            ...
            self.warnings.append("⚠️ Safety tool not installed (pip install safety)")
    ...
    def check_git_configuration(self) -> None:
        ...
        if missing_patterns:
            self.warnings.append(f"⚠️ .gitignore missing patterns: {', '.join(missing_patterns)}")
        else:
        ...
        for filename in sensitive_files:
            ...
            if file_path.exists():
                ...
                if permissions[1:] != '00':  # Should be -rw-------
                    self.warnings.append(f"⚠️ {filename} has permissive permissions: {permissions}")
    ...
    def check_database_security(self) -> None:
        ...
        if db_files:
            try:
                ...
                if result.returncode != 0:
                    self.warnings.append(f"⚠️ Database files not gitignored: {[f.name for f in db_files]}")
                else:
                    print("✅ Database files are properly gitignored")
                    
            except subprocess.SubprocessError:
                self.warnings.append("⚠️ Could not check if database files are gitignored")
